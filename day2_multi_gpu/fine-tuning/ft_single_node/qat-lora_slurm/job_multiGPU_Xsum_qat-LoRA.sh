#!/bin/bash -e
#SBATCH --job-name=ft-llama3-1B-qat-lora-4gpu_xsum
...
CONFIG_FILE="${MyWD}/configs/qat-lora/llama3_2_1B_qat-lora_multi_device_XSum.yaml"
...
#tune run --nnodes $N --nproc_per_node $nproc_perN --standalone lora_finetune_distributed --config "${CONFIG_FILE}" checkpointer.output_dir="${OUTPUT_DIR}" output_dir="${LOGGING_DIR}" 
EOF
